...
void CFuncVehicle::Blocked(CBaseEntity *pOther)
{
	...
	float minx = std::fmin(vBackLeft.x, vBackRight.x);
	float miny = std::fmin(vBackLeft.y, vBackRight.y);
	float maxx = std::fmax(vBackLeft.x, vBackRight.x);
	float maxy = std::fmax(vBackLeft.y, vBackRight.y);

	float minz = pev->origin.z;
#ifdef REGAMEDLL_FIXES
	float maxz = pev->origin.z + (2 * Q_abs(pev->mins.z - pev->maxs.z));
#else
	float maxz = pev->origin.z + (2 * std::abs(int(pev->mins.z - pev->maxs.z)));
#endif
...
#else
	float flpitch = VEHICLE_STARTPITCH + (std::abs(int(pev->speed)) * (VEHICLE_MAXPITCH - VEHICLE_STARTPITCH) / VEHICLE_MAXSPEED);
#endif
...
void CFuncVehicle::CheckTurning()
{
	...
	if (pev->speed > 0)
	{
		int iCountTurn = std::abs(m_iTurnAngle);
...
inline float UTIL_FixupAngle(float v)
{
	float angle = v;
...
void CFuncVehicle::NearestPath()
{
	...
	CBaseEntity *pTrackBase = nullptr;
	CPathTrack *pNearest = nullptr;
	float dist;
	float closest = 1024.0f;

	while ((pTrack = dynamic_cast<CPathTrack *>(UTIL_FindEntityInSphere(pTrack, pev->origin, 1024.0f))))
	{
...
CFuncVehicle *CFuncVehicle::Instance(edict_t *pEdict)
{
	...
	{
		return (CFuncVehicle*)GET_PRIVATE(pEdict);
	}
...
void CFuncVehicleControls::Find()
{
	...
	while ((pVehicle = UTIL_FindEntityByString(pVehicle, "targetname", STRING(pev->target))))
	{
		if (FClassnameIs(pVehicle->pev, "func_vehicle"))
			break;
	}

	if (pVehicle->pev == nullptr)
	{
		ALERT(at_console, "No vehicle %s\n", STRING(pev->target));
		return;
	}
	CFuncVehicle *pVehicleFuncVehicle = (CFuncVehicle*)pVehicle;

	pVehicleFuncVehicle->SetControls(pev);
	UTIL_Remove(this);
}
